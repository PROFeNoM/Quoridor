...
void initialize(enum color_t id, struct graph_t* graph, size_t num_walls)
{
  ...
  if (!is_initialized) {
    ...
    player.dim = sqrt(graph->num_vertices);
...
enum color_t other_player(enum color_t id)
{
  return (id+1) %2;
}

/*int is_first_step_forward(enum color_t id)
{
  size_t starting_positions[player.graph->num_vertices];
  size_t nb_pos = 0;
  ...
  //searching for available positions
  for (size_t index = 0; index < player.graph->num_vertices; index++) {
    if (is_owned(player.graph, id, index) == 1) {
      starting_positions[nb_pos++] = index;
    }
  }

  int is_not_starting_position(size_t pos)
  {
    for (size_t i = 0; i < nb_pos;
  }

  for (size_t ind = 0; ind < nb_pos; ind++) {
    if (is_connected(player.graph, starting_positions[ind], player.position[id]) && 
  }  
}
*/
...
struct move_t get_new_move()
{
  ...
  size_t position_player2 = player.position[other_player(player.id)];
  
  static int never_gone_out_of_spawn = 1;
  if (never_gone_out_of_spawn) {
    if (is_first_step_forward(other_player(player.id))) {
      never_gone_out_of_spawn = 0;
      struct edge_t e1;
      struct edge_t e2;
      e1.fr = position_player2
    }
  }*/
